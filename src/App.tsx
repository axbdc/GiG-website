...
// RESERVATION OVERLAY
// ═══════════════════════════════════════════════════
function ReservationOverlay({ isOpen, onClose, lang }: { isOpen: boolean; onClose: () => void; lang: Language }) {
  const t = copy[lang].reservationOverlay;
  const [submitted, setSubmitted] = useState(false);
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", date: "", time: "", guests: "", message: "" });
  ...
  const inputClass = "w-full rounded-xl border border-[#18352a]/18 bg-white/60 px-4 py-3.5 text-sm text-[#18352a] placeholder:text-[#18352a]/35 outline-none transition focus:border-[#6f8f72] focus:ring-1 focus:ring-[#6f8f72]/40";

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-label={t.title}>
      <div className="absolute inset-0 bg-[#18352a]/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl bg-[#f7f0e3] shadow-2xl reservation-panel">
        {submitted ? (
          <div className="flex flex-col items-center gap-4 px-8 py-16 text-center">
            ...
            <button onClick={onClose} className="mt-2 rounded-full bg-[#18352a] px-8 py-3 text-sm font-bold uppercase tracking-[0.14em] text-[#fff8ea] transition hover:bg-[#6f8f72]">
              {t.closeSuccess}
            </button>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 px-6 pb-10 pt-6">
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.nameLabel} <span className="text-[#18352a]/30">{t.required}</span></label>
                <input type="text" required className={inputClass} placeholder={t.namePlaceholder} value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.emailLabel} <span className="text-[#18352a]/30">{t.required}</span></label>
                <input type="email" required className={inputClass} placeholder={t.emailPlaceholder} value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.phoneLabel}</label>
                <input type="tel" className={inputClass} placeholder={t.phonePlaceholder} value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.dateLabel} <span className="text-[#18352a]/30">{t.required}</span></label>
                  <input type="date" required className={inputClass} value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.timeLabel} <span className="text-[#18352a]/30">{t.required}</span></label>
                  <input type="time" required min="10:00" max="16:00" className={inputClass} value={formData.time} onChange={(e) => setFormData({ ...formData, time: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.guestsLabel} <span className="text-[#18352a]/30">{t.required}</span></label>
                <select required className={inputClass + " appearance-none cursor-pointer"} value={formData.guests} onChange={(e) => setFormData({ ...formData, guests: e.target.value })}>
                  <option value="">{t.guestsPlaceholder}</option>
                  {Array.from({ length: 20 }, (_, i) => i + 1).map((n) => (
                    <option key={n} value={n}>{n} {n === 1 ? (lang === "pt" ? "pessoa" : "person") : (lang === "pt" ? "pessoas" : "people")}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-[0.18em] text-[#6f8f72]">{t.messageLabel}</label>
                <textarea rows={3} className={inputClass} placeholder={t.messagePlaceholder} value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} />
              </div>
              <button type="submit" className="w-full rounded-full bg-[#18352a] py-4 text-sm font-bold uppercase tracking-[0.16em] text-[#fff8ea] transition hover:bg-[#6f8f72]">
                {t.submitButton}
              </button>
